use fender::{stdlib::print_func, FenderTypeSystem, FenderValue};
...
fn main() {
    ...
    let a = main.create_variable();
    main.assign_value(a, FenderValue::Int(3).into()).unwrap();

    let mut lambda = FunctionWriter::new_capturing(0, vec![a]);
    lambda
        .evaluate_expression(Expression::StaticFunctionCall(
            print_func,
            vec![Expression::Variable(lambda.captured_stack_offset(0))],
        ))
        .unwrap();
    let lambda = writer.include_function(lambda);

    let f = main.create_variable();
    main.assign_value(f, Expression::FunctionCapture(lambda))
        .unwrap();

    main.evaluate_expression(Expression::DynamicFunctionCall(
